...
// ── Account Interactions (기업 접촉 기록) ─────────────

export async function getInteractions(accountId) {
  return request(`/accounts/${accountId}/interactions`);
}

export async function addInteraction(accountId, data) {
  return request(`/accounts/${accountId}/interactions`, {
    method: "POST",
    body: JSON.stringify({ ...data, botName: "erp-mcp" }),
  });
...
// ── Project Groups (묶음 세금계산서) ────────────────────

export async function getProjectGroup(id) {
  const res = await request(`/project-groups/${id}`);
  return res.group || res;
}

export async function updateProjectGroup(id, data) {
  const res = await request(`/project-groups/${id}`, {
    method: "PATCH",
    body: JSON.stringify({ ...data, botName: "erp-mcp" }),
  });
...
// ── Policy Documents (내부 정책 문서: 교육 단가 등, 프로젝트 무관) ─────

export async function listPolicyDocuments({ category, currentOnly } = {}) {
  const qs = new URLSearchParams();
  if (category) qs.set("category", category);
  if (currentOnly) qs.set("current", "1");
  const res = await request(`/policies${qs.toString() ? `?${qs}` : ""}`);
  return res.documents || [];
}

/** 현행 정책 문서 본문(md) 또는 메타. format=md면 텍스트, 없으면 { category, version, documents } */
export async function getCurrentPolicy({ category = "pricing", format } = {}) {
  const qs = new URLSearchParams({ category });
  if (format) qs.set("format", format);
  const res = await fetch(`${BASE_URL}/policies/current?${qs}`, { headers: { "x-api-key": apiKey } });
  if (!res.ok) {
    const body = await res.text();
    throw new Error(`ERP API ${res.status}: ${body}`);
  }
  if (format === "md") {
    return { text: await res.text(), version: res.headers.get("x-policy-version"), documentId: res.headers.get("x-policy-document-id") };
  }
  return res.json();
}

export async function addPolicyLink(data) {
  const res = await request(`/policies`, { method: "POST", body: JSON.stringify({ ...data, botName: "erp-mcp" }) });
  return res.document || res;
}

export async function updatePolicyDocument(itemId, data) {
  const res = await request(`/policies`, { method: "PATCH", body: JSON.stringify({ itemId, ...data, botName: "erp-mcp" }) });
  return res.document || res;
}

// ── Cases (수강생 결과물 사례 인덱스 — 데모 제작 스킬의 청사진 후보) ─────

/** 목록. 서버가 limit/offset/total 을 주므로 전체가 필요하면 offset 을 넘겨 이어 읽는다. */
export async function listCases({ type, job, tag, source, cohort, projectId, q, consent, excludeDemo = true, includeIncomplete = false, limit = 50, offset = 0 } = {}) {
  const qs = new URLSearchParams();
  ...
  if (q) qs.set("q", q);
  if (consent) qs.set("consent", consent);
  qs.set("excludeDemo", excludeDemo ? "1" : "0");
  if (includeIncomplete) qs.set("includeIncomplete", "1");
  qs.set("limit", String(limit));
  qs.set("offset", String(offset));
  return request(`/cases?${qs}`); // { total, limit, offset, cases }
}

/** 상세. key = case_key(hr2-01-a) 또는 uuid. 첨부는 path 목록 — 실물은 BASE_URL + path 에 x-api-key 로 GET. */
export async function getCase(key) {
  const res = await request(`/cases/${encodeURIComponent(key)}`);
  return res.case || res;
}
...
export async function getActivity({ limit, entityType, projectId, dateFrom, dateTo } = {}) {
  ...
  return { logs: res.logs || [], nextCursor: res.nextCursor || null };
}
