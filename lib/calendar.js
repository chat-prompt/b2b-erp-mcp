...
/**
 * 두 캘린더(owner + shared GPTers 교육 캘린더)에서 프로젝트명 매칭 이벤트 검색.
 */
export async function searchProjectEvents({ projectName, timeMin, timeMax, fuzzyKeywords = [] }) {
  const calendar = await getCalendar();
  const calendarIds = [OWNER_CALENDAR_ID, EDUCATION_CALENDAR_ID];
  const all = [];
  for (const calendarId of calendarIds) {
    try {
      const res = await calendar.events.list({
        calendarId,
        q: projectName,
        timeMin: timeMin || "2025-01-01T00:00:00+09:00",
        timeMax: timeMax || "2027-12-31T23:59:59+09:00",
        singleEvents: true,
        orderBy: "startTime",
        maxResults: 100,
      });
      ...
      for (const e of items) {
        all.push({ ...e, _calendar: calendarId });
      }
    } catch (err) {
      // 캘린더 접근 실패는 무시 (권한 부족 등)
      console.error(`Calendar ${calendarId} 접근 실패:`, err.message);
    }
  }
  // dedupe by id (같은 이벤트가 두 캘린더에 attendee로 들어가있을 수 있음)
  const seen = new Set();
  const unique = [];
  for (const e of all) {
    if (seen.has(e.id)) continue;
    seen.add(e.id);
    // fuzzy match: summary가 projectName 또는 키워드 포함
    const summary = e.summary || "";
    const matchesName = summary.includes(projectName) || (fuzzyKeywords.length > 0 && fuzzyKeywords.some((kw) => summary.includes(kw)));
    if (matchesName) unique.push(e);
  }
  return unique;
}
